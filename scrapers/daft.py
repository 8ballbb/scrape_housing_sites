...
def scrape_daft_for_sale(
    ...
    df: Union[pd.DataFrame, None]=None) -> pd.DataFrame:
    ...
    if df is not None:
        ...
        df.loc[df["href"].isin(set(df["href"]).difference(df_new["href"])), "sold"] = 1
        # subset of new listing
        df_new = df_new.loc[df_new["href"].isin(set(df_new["href"]).difference(df["href"]))]
    tqdm.pandas()
    df_new["date_posted"], df_new["views"], df_new["desc"], df_new["features"], df_new["lng"], df_new["lat"] = zip(*df_new["href"].progress_apply(scrape_property_page))
